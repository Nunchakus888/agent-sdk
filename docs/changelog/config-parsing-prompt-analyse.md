...
This document summarizes the optimizations made to the configuration parsing logic based on the [sop.json](configs/sop.json) configuration structure.
...
- **Fixed/Default Behavior**: `retrieve_knowledge_url`, tools - No LLM processing, automatically appended to instructions
- **LLM-Parsed Config**: `instructions`, `timers`, `need_greeting`, `constraints` and so on ...- Enhanced by LLM
...
**Note**: Knowledge base section is NOT generated by LLM. It's automatically appended after LLM processing if `retrieve_knowledge_url` exists.
...
**Auto-Construction**:
If `retrieve_knowledge_url` is missing but `retrieve_knowledge_url` exists:
```python

```

**Auto-Appending to Instructions**:
After LLM processing, if `retrieve_knowledge_url` exists, the following section is automatically appended:
```markdown
## KNOWLEDGE BASE
- You have access to a knowledge base via retrieve_knowledge tool
- Use it proactively when users ask questions about products, services, or domain knowledge
...
2. Extract fixed config (retrieve_knowledge_url)
...
6. Auto-append knowledge base section to instructions (if retrieve_knowledge_url exists)
...
4. **Flexibility**: Auto-construction of retrieve_knowledge_url from configs
...
│  │  - 时机: 配置加载时（一次性）                                      │   │
│  │  - 输入: raw_config                                              │   │
│  │  - 输出: instructions, timers, need_greeting, constraints        │   │
│  │  - 缓存: 结果存入 DB，避免重复调用 LLM                            │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│       │                                                                 │
│       ▼                                                                 │
│  WorkflowConfigSchema (完整配置)                                        │
│       │                                                                 │
│       ▼                                                                 │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  SystemPromptBuilder                                             │   │
│  │  - 时机: Agent 创建时（每次会话）                                  │   │
│  │  - 输入: WorkflowConfigSchema                                    │   │
│  │  - 输出: 7 层结构化 system prompt                                 │   │
│  │  - 特点: 纯模板渲染，无 LLM 调用                                  │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│       │                                                                 │
│       ▼                                                                 │
...
| File | Description |
|------|-------------|
| `api/services/v2/config_enhancer.py` | ConfigEnhancer 类 - LLM 增强配置 |
| `api/services/v2/session_manager.py` | SessionManager._llm_enhance - 调用 ConfigEnhancer |
| `bu_agent_sdk/prompts/templates.py` | 共享模板常量 |
| `bu_agent_sdk/prompts/builder.py` | SystemPromptBuilder - 7 层 prompt 构建 |
| `bu_agent_sdk/schemas.py` | WorkflowConfigSchema - 配置数据模型 |
...
# Returns: {"instructions": "...", "need_greeting": "...", "timers": [], "constraints": "..."}
```
...
```python
JAILBREAK_PATTERNS = [
    (r"you are now|forget you are|pretend to be", "role_manipulation"),
    (r"ignore previous|disregard rules|forget instructions", "instruction_override"),
    (r"execute|run command|access system|shell", "system_access"),
    (r"send to|export|leak|exfiltrate", "data_exfiltration"),
]
```
...
```bash
pytest tests/test_config_enhancer.py -v
```

Test coverage:
- Prompt building (empty config, with basic_settings, with existing instructions)
- Response parsing (valid JSON, with code blocks, invalid JSON fallback)
- Jailbreak detection (role manipulation, instruction override, system access, no false positives)
- Knowledge base appending
- Fallback handling
